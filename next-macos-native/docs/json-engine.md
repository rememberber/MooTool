...
- 单次正文最多 10 MB，请求/结果最多 16 MB，JSON 最多 10 万节点、128 层；结构树和查询/查找结果最多 2 万项。父进程默认 3 秒超时；辅助程序自身另设 4 秒终止计时，即使主应用异常退出也会停止耗时表达式。Worker 侧用户可见错误随请求 `language` 三语（`JSONDispatch.js` / `JSONTools.js`）；fast-xml-parser 等第三方库的英文 `msg` 仍可能透传。
...
- Monaco 的全部快捷操作、折叠和编辑装饰尚未移植。文档库已支持镜像到 `json-vault/`、FSEvents 监听、Finder 定位与 Git 面板（Diff、丢弃、合并/变基与冲突处理）；与 Electron 的差异见 [parity](parity.md)。
