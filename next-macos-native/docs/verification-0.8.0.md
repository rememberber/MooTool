...
验收日期：2026-09-14。环境为 macOS 26.7（25G229）、Intel x86_64、Swift 6.2.3 / Command Line Tools 26。应用最低运行版本为 macOS 14。

## 功能与布局

- 53 组 Core 测试全部通过。文本对比用例对照 Electron 样例检查 UTF-16 字符范围、行增删改计数、三行上下文统一补丁、忽略空白、末尾空行、长行回退和旧工作区读取；原有格式化、附件、随手记、JSON、HTTP 与文档库用例也通过。
