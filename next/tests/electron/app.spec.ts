...
test('formats JSON and completes history and Vault workflows', async () => {
  ...
  await expect(vaultNode).toBeVisible()
  await vaultSearchContent.uncheck()
  await expect(vaultNode).toHaveCount(0)
  await vaultSearch.fill('e2e-sample')
  await expect(vaultNode).toBeVisible()
...
test('runs P3 regex, Cron and text Diff workflows with persistent favorites', async () => {
  ...
  await expect(mainPage.locator('.diff-status')).toContainText('共发现 1 处差异')
  await expect(mainPage.locator('.diff-editor-grid .cm-diff-character-changed')).toHaveCount(2)
...
test('runs P4 color favorites, image persistence and PDF workspace', async () => {
  ...
  expect(await mainPage.locator('.color-palette-panel').evaluate((panel) => {
    const select = panel.querySelector('select')!.getBoundingClientRect()
    const colors = panel.querySelector('.theme-main-colors')!.getBoundingClientRect()
    return colors.top - select.bottom
  ...
  await saveColorFavorite.getByRole('button', { name: '新建收藏夹' }).click()
  const createColorFolder = mainPage.getByRole('dialog', { name: '新建收藏夹' })
  await createColorFolder.getByLabel('请输入收藏夹名称').fill('E2E 收藏夹')
  await createColorFolder.getByRole('button', { name: '添加', exact: true }).click()
  await expect(saveColorFavorite.getByLabel('收藏夹')).toHaveValue(/\d+/)
  await expect(saveColorFavorite.getByLabel('收藏夹').locator('option:checked')).toHaveText('E2E 收藏夹')
  ...
  const previewImage = mainPage.locator('.image-canvas img')
  const previewCanvas = mainPage.locator('.image-canvas')
  await expect(previewImage).toBeVisible()
  await expect.poll(() => previewImage.evaluate((image) => image.getBoundingClientRect().width)).toBeGreaterThan(8)
  await mainPage.getByRole('button', { name: '原始尺寸' }).click()
  await expect.poll(() => previewImage.evaluate((image) => image.getBoundingClientRect().width)).toBeCloseTo(8, 0)
  await mainPage.getByRole('button', { name: '适应窗口' }).click()
  const fitWidth = await previewImage.evaluate((image) => image.getBoundingClientRect().width)
  await previewCanvas.dispatchEvent('wheel', { ctrlKey: true, deltaY: -10 })
  await expect.poll(() => previewImage.evaluate((image) => image.getBoundingClientRect().width)).toBeGreaterThan(fitWidth)
...
test('runs P5 HTTP requests and persists the request collection', async () => {
  ...
  try {
    ...
    await expect(mainPage.getByTestId('http-timeout')).toHaveValue('30000')
    await mainPage.getByTestId('http-timeout').fill('45000')
    await mainPage.getByTestId('http-timeout').blur()
    await expect.poll(() => mainPage.evaluate(async () => (await window.mootool.getSettings()).network.requestTimeoutMs)).toBe(45_000)
    ...
    await expectTextEditorChrome(mainPage.locator('.http-response-code-editor'))
    await mainPage.locator('.http-status').getByRole('button', { name: '查找' }).click()
    const findBar = mainPage.locator('.http-findbar')
    await expect(findBar).toBeVisible()
    await findBar.getByRole('textbox', { name: '查找', exact: true }).fill('echo')
    await findBar.getByRole('button', { name: '查找', exact: true }).click()
    await expect(findBar).toContainText('共找到:')
    await findBar.getByRole('button', { name: '关闭', exact: true }).click()
...
test('runs P5 Hosts, translation records, network and system workflows', async () => {
  ...
  const hostEditor = mainPage.getByTestId('host-content')
  await hostEditor.fill('# E2E profile\n127.0.0.1 e2e.mootool.local\n127.0.0.2 E2E.mootool.local\n')
  ...
  const hostFindBar = mainPage.locator('.host-findbar')
  const hostFindInput = hostFindBar.getByRole('textbox', { name: '查找', exact: true })
  await expect(hostFindInput).toBeFocused()
  await hostFindInput.fill('e2e.mootool.local')
  await expect(hostFindBar).toContainText('共找到: 2')
  await hostFindBar.getByRole('checkbox', { name: '区分大小写' }).check()
  await expect(hostFindBar).toContainText('共找到: 1')
  await hostFindBar.getByRole('checkbox', { name: '区分大小写' }).uncheck()
  await hostFindInput.fill('127\\.0\\.0\\.\\d')
  await hostFindBar.getByRole('checkbox', { name: '使用正则' }).check()
  await expect(hostFindBar).toContainText('共找到: 2')
  await hostFindBar.getByRole('textbox', { name: '替换为', exact: true }).fill('10.0.0.1')
  await hostFindBar.getByRole('button', { name: '替换全部', exact: true }).click()
  await expect(hostFindBar).toContainText('已替换: 2')
  await expect(hostEditor).toContainText('10.0.0.1 e2e.mootool.local')
  await expect(hostEditor).toContainText('10.0.0.1 E2E.mootool.local')
  await hostFindBar.getByRole('button', { name: '关闭', exact: true }).click()
  await mainPage.getByRole('button', { name: '保存', exact: true }).click()
  const hostProfile = mainPage.locator('.host-profile').filter({ hasText: 'E2E development' })
  await expect(hostProfile).toBeVisible()
  const hostSearch = mainPage.locator('.host-profiles').getByRole('textbox', { name: '搜索', exact: true })
  const hostSearchContent = mainPage.locator('.host-profiles').getByRole('checkbox', { name: '搜索正文' })
  await hostSearch.fill('E2E.mootool.local')
  await expect(hostProfile).toBeVisible()
  await hostSearchContent.uncheck()
  await expect(hostProfile).toHaveCount(0)
  await hostSearch.fill('E2E development')
  await expect(hostProfile).toBeVisible()
...
test('supports column editing in the Quick Note editor', async () => {
  ...
  await expect.poll(() => editor.evaluate((element) => (element as HTMLElement).innerText)).toBe('xaa\nxbb')
  await mainPage.getByRole('button', { name: '保存', exact: true }).click()
})
