...
export function ProtobufTool() {
  ...
  return (
    <section className="tool-page p4-tool">
      <ToolPageHeader title={t('protobuf.title')} />
      <div className="local-tool-shell protobuf-workspace">
        <ToolTabs tabs={[{ id: 'json', label: t('protobuf.tab.json') }, { id: 'wire', label: t('protobuf.tab.wire') }, { id: 'convert', label: t('protobuf.tab.convert') }]} active={tab} onChange={setTab} windowDrag actions={<button className="toolbar-button" type="button" onClick={() => setHistoryOpen(true)}><History size={14} />{t('common.action.history')}</button>} />
