...
import type { ImageVectorizeOptions } from '@/shared/contracts/images'
...
export function ImageSvgDialog({ open, count, onClose, onConfirm }: { open: boolean; count: number; onClose: () => void; onConfirm: (options: ImageVectorizeOptions) => void }) {
  const { t } = useI18n()
  const [preset, setPreset] = useState<ImageVectorizeOptions['preset']>('poster')
  const [colorCount, setColorCount] = useState(16)
  const [detail, setDetail] = useState<ImageVectorizeOptions['detail']>('medium')
  const [filterSpeckle, setFilterSpeckle] = useState(8)
  return <Dialog title={t('image.svgTitle')} open={open} width={540} onClose={onClose} footer={<><button className="dialog-button" type="button" onClick={onClose}>{t('common.cancel')}</button><button className="dialog-button dialog-button--primary" type="button" onClick={() => onConfirm({ preset, colorCount, detail, filterSpeckle })}>{t('image.svgStart')}</button></>}><div className="image-options"><p>{t('image.selectedCount', { count: String(count) })}</p><div className="image-option-grid"><label><span>{t('image.svgPreset')}</span><select value={preset} onChange={(event) => setPreset(event.target.value as ImageVectorizeOptions['preset'])}><option value="poster">{t('image.svgPreset.poster')}</option><option value="photo">{t('image.svgPreset.photo')}</option><option value="bw">{t('image.svgPreset.bw')}</option></select></label><label><span>{t('image.svgColors')}</span><input type="number" min={2} max={64} disabled={preset === 'bw'} value={colorCount} onChange={(event) => setColorCount(Math.min(64, Math.max(2, Number(event.target.value) || 2)))} /></label><label><span>{t('image.svgDetail')}</span><select value={detail} onChange={(event) => setDetail(event.target.value as ImageVectorizeOptions['detail'])}><option value="low">{t('image.svgDetail.low')}</option><option value="medium">{t('image.svgDetail.medium')}</option><option value="high">{t('image.svgDetail.high')}</option></select></label><label><span>{t('image.svgSpeckle')}</span><input type="number" min={0} max={128} value={filterSpeckle} onChange={(event) => setFilterSpeckle(Math.min(128, Math.max(0, Number(event.target.value) || 0)))} /></label></div><p className="image-svg-hint">{t('image.svgHint')}</p></div></Dialog>
}
