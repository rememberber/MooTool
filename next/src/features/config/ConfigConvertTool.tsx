...
export function ConfigConvertTool() {
  ...
  return (
    <section className="tool-page p3-tool">
      <ToolPageHeader title={t('config.title')} />
      <div className="local-tool-shell config-workspace">
        <ToolTabs tabs={[{ id: 'convert', label: t('config.tab.convert') }, { id: 'validate', label: t('config.tab.validate') }]} active={tab} onChange={setTab} windowDrag actions={<button className="toolbar-button" type="button" onClick={() => setHistoryOpen(true)}><History size={14} />{t('common.action.history')}</button>} />
