  ...
  return (
    <section className="tool-page p3-tool">
      ...
      <div className="local-tool-shell config-workspace">
        ...
        {tab === 'convert' ? <div className="io-workspace"><TextPane label={t('config.properties')} value={properties} onChange={setProperties} /><div className="io-actions"><button className="io-action-button" type="button" onClick={() => convert('toYaml')}><ArrowRight size={15} />{t('config.toYaml')}</button><button className="io-action-button" type="button" onClick={() => convert('toProperties')}><ArrowLeft size={15} />{t('config.toProperties')}</button></div><TextPane label={t('config.yaml')} value={yaml} onChange={setYaml} /></div> : <div className="yaml-validate-layout"><TextPane label={t('config.yaml')} value={validateSource} onChange={setValidateSource} /><div className="validate-actions"><button className="primary-command" type="button" onClick={validate}>{t('config.validate')}</button><button className="panel-command" type="button" onClick={format}><WandSparkles size={14} />{t('config.format')}</button></div><output className={valid === false ? 'validation-output validation-output--error' : 'validation-output'}>{validation || t('config.validate')}</output></div>}
      </div>
