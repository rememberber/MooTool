    ...
    <section className="tool-page p5-tool translation-tool-page">
      ...
      <div className="local-tool-shell translation-workspace">
        ...
        {tab === 'translate' && <div className="translation-main">
          ...
          <div className="translation-editor-grid"><textarea data-testid="translation-source" value={source} placeholder={t('translation.sourcePlaceholder')} spellCheck={false} onChange={(event) => { restoredSource.current = null; setSource(event.target.value) }} /><div className="translation-result"><textarea data-testid="translation-result" value={translating ? t('translation.translating') : target} readOnly spellCheck={false} /><footer>{providerUsed && <span><Languages size={13} />{providerUsed === 'google' ? 'Google' : 'Bing'}{fallbackUsed ? ` · ${t('translation.fallback')}` : ''}</span>}<span>{source.length} / 50000</span></footer></div></div>
        </div>}
...
function WordBook({ onApply, onRetranslate }: { onApply: (word: TranslationWord) => void; onRetranslate: (word: TranslationWord) => Promise<TranslationWord> }) {
  ...
  return <div className="translation-record-layout"><aside><div className="compact-search"><Search size={13} /><input value={query} placeholder={t('translation.searchWords')} onChange={(event) => setQuery(event.target.value)} /></div><div className="translation-record-list">{items.map((item) => <button className={item.id === selected?.id ? 'translation-record translation-record--active' : 'translation-record'} type="button" key={item.id} onClick={() => setSelected(item)}><strong>{item.sourceText}</strong><span>{item.targetText}</span></button>)}</div><footer><button className="icon-button" type="button" aria-label={t('common.add')} onClick={() => setSelected({ id: 0, sourceText: '', targetText: '', sourceLang: 'auto', targetLang: 'zh-CN', remark: '', createTime: '', modifiedTime: '' })}><Plus size={14} /></button><button className="icon-button icon-button--danger" type="button" disabled={!selected?.id} aria-label={t('common.action.delete')} onClick={() => { void remove() }}><Trash2 size={14} /></button></footer></aside><main>{selected ? <><header><span>{languageLabel(selected.sourceLang, t)} → {languageLabel(selected.targetLang, t)}</span><div><button className="toolbar-button" type="button" onClick={() => onApply(selected)}><Languages size={14} />{t('translation.apply')}</button><button className="toolbar-button" type="button" onClick={() => { void onRetranslate(selected).then((word) => { setSelected(word); void load() }) }}><History size={14} />{t('translation.retranslate')}</button></div></header><textarea value={selected.sourceText} placeholder={t('translation.sourcePlaceholder')} onChange={(event) => setSelected({ ...selected, sourceText: event.target.value })} /><textarea value={selected.targetText} placeholder={t('translation.targetPlaceholder')} onChange={(event) => setSelected({ ...selected, targetText: event.target.value })} /><input value={selected.remark} placeholder={t('translation.remark')} onChange={(event) => setSelected({ ...selected, remark: event.target.value })} /><button className="dialog-button" type="button" onClick={() => { void save() }}><Save size={14} />{t('common.save')}</button></> : <div className="history-empty">{t('translation.wordEmpty')}</div>}</main></div>
}
