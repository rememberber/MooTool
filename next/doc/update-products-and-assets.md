...
4. 将工件及 Electron Builder 生成的 `latest*.yml`、blockmap 更新元数据一并上传到对应产品标签的 GitHub Release。
...
7. 清单合并后，旧版本客户端会自动选中与自身系统和架构匹配的文件；默认在后台静默下载，用户关闭该设置后则等待手动下载。
8. 下载完成后，客户端在侧边导航底部显示“安装并重启”，点击后由 `electron-updater` 校验并安装对应平台产物。
