...
| 环境变量 | 用户/系统变量直接新增、编辑、删除；Java Properties Tab、表格、导出 | 用户/系统变量直接新增、编辑、删除；当前进程/Electron 运行属性只读视图、搜索、复制、刷新和完整导出 | 待对齐验收 |
...
- [x] 环境变量和系统信息保留 Java 版 Tab、表格/键值结构与刷新、复制、导出操作；两版均支持直接修改用户/系统环境变量。
