...
P6 桌面通道：`com.rememberber.mootool.next.flutter/desktop`。macOS `MainFlutterWindow.swift` 提供：IOPM 防休眠、NSPasteboard 剪贴板图片、NSStatusItem 托盘、关闭 ask/hide/quit。截图/取色仍返回 `UNIMPLEMENTED`。本机无完整 Xcode，Swift 未在本机编译运行。
