...
| ID | 缺口 | 用户如何完成原任务 |
| --- | --- | --- |
| 多窗口 | 第二 Flutter engine 未实现；分离按钮不再假装成功 | 继续在主窗口工作 |
...
| F24 | 任意加密 PDF / 真实页对象复制 | 仅 SimplePdf 明文文本拆合；非该格式会拒绝 |
| F25 | CPU% | 看系统监视器 |
| A03 | 实机安装/升级/卸载、五产品并存实测 | 等 CI 产物后人工验收 |
| 发行 | 本机有 Xcode；p7b 与 2026-09-09 审查隔离目录已产出未签名 DMG；未写 update-manifest 节点 | 在 Finder 中右键打开；Windows/Linux 仍未本机构建 |
