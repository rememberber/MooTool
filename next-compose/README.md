...
**当前状态：0.1.0 开发切片。** 已有独立 Gradle Wrapper 工程，本机可构建、测试，并生成带自有 runtime 的 macOS app-image。已实现桌面壳、26 个入口、modern 明暗、基础设置，JSON 切片，随手记切片（文档库与 24 项快速替换），文本对比，格式化，配置转换，Protobuf，编码解码，加解密/随机，二维码，UA 分析，正则，Cron，时间转换，计算器，P5 网络/系统工具（HTTP、Host、网络、环境变量、翻译、系统信息），以及代码运行。这不是完整 P2/F04/F01，也不是三平台发行验收。
