...
**当前状态：0.1.0 开发切片。** 已有独立 Gradle Wrapper 工程，本机可构建、测试，并生成带自有 runtime 的 macOS app-image。已实现桌面壳、26 个入口、modern 明暗、基础设置，JSON 切片，时间转换，以及计算器（四则/进制/GCD/LCM/排列组合）。其余工具明确显示尚未实现。这不是完整 P2/F04，也不是三平台发行验收。
...
常用操作：`⌘/Ctrl+K` 搜索工具，`⌘/Ctrl+,` 打开设置。JSON 支持格式化、压缩、查找替换、导入导出、历史和左侧 Vault。时间转换支持显式秒/毫秒、IANA 时区、历史和大屏时钟（单位语义见 [DIFF-001](docs/diff/001-time-explicit-unit.md)）。计算器支持四则表达式、2/10/16 进制、GCD/LCM 与排列组合，不用 eval。
