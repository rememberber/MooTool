...
**当前状态：0.1.0 开发切片。** 已有独立 Gradle Wrapper 工程，本机可构建、测试，并生成带自有 runtime 的 macOS app-image。已实现桌面壳、26 个入口、modern 明暗、基础设置，JSON 切片，编码解码，UA 分析，时间转换，以及计算器。其余工具明确显示尚未实现。这不是完整 P2/F04，也不是三平台发行验收。
