...
| mcp-stdio-vault | 同上 | MCP 子进程调用 vault 工具 | `AiIntegrationVaultMcpConnectionTest` notes/json（[DIFF-463](../diff/463-vault-mcp-stdio-e2e-symlink-access.md)、[DIFF-464](../diff/464-vault-access-schema-editor-ime-prep.md)）；notes read 分页 + list/read 联调见 [DIFF-565](../diff/565-f15-regex-worker-css-vault-mcp-p7-slice.md)；双库 json read offset 见 [DIFF-569](../diff/569-time-host-mcp-css-slice.md)；双库 notes read offset 见 [DIFF-570](../diff/570-config-mcp-css-slice.md) |
