...
/** modern `.icon-ghost`：`--desktop-control-radius` 9dp（DIFF-505），非固定 6dp。 */
class GhostButtonModernCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun modernGhostButtonRendersSearchGlyph() = runDesktopComposeUiTest(width = 120, height = 120) {
        setContent {
            AppMooTheme(
                AppMooThemeInputs(
                ...
                Column(
                    Modifier.fillMaxSize().background(colors.sidebar).mooSidebarBackground().padding(8.dp)
                ) {
                    MooGhostButton(label = "Search", onClick = {}) {
        ...
        dir.mkdirs()
        val file = File(dir, "icon-ghost-modern-search.png")
