...
/** F22 调色板屏幕取色钮 Compose Tab 焦点帧（非产品主窗）。 */
class ColorScreenPickCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun captureColorScreenPickTabFocus() = runDesktopComposeUiTest(width = 400, height = 88) {
        ...
        waitForIdle()
        val file = File(dir, "183-compose-color-screen-pick-tab-focus.png")
        val image = onRoot().captureToImage().toAwtImage()
        assertTrue(ImageIO.write(image, "png", file))
        assertTrue(countRingPixels(image, expected) >= 8, "color screen pick focus ring")
    }
