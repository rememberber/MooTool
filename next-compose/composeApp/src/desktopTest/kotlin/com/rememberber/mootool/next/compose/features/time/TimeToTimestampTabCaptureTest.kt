...
/** F18 时间「转时间戳」钮 Compose Tab 焦点帧（非产品主窗）。 */
class TimeToTimestampTabCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun captureTimeToTimestampTabFocus() = runDesktopComposeUiTest(width = 400, height = 72) {
            ...
            MooTheme(preference = ThemePreference.Light, systemDark = false, interfaceStyle = "modern") {
                ...
                Row(
                    Modifier
                        ...
                        .mooTimeConvertActions()
                        .padding(horizontal = 8.dp),
                ) {
                    MooButton(zh.t("time.toLocal"), p5Toolbar = true, onClick = {})
                    MooButton(
                        zh.t("time.toTimestamp"),
                        p5Toolbar = true,
                        onClick = {},
        ...
        waitForIdle()
        val file = File(dir, "193-compose-time-to-timestamp-tab-focus.png")
        val image = onRoot().captureToImage().toAwtImage()
        assertTrue(ImageIO.write(image, "png", file))
        assertTrue(countRingPixels(image, expected) >= 8, "time toTimestamp focus ring")
    }
