...
/** F01 随手记「列编辑」闩锁钮 Compose Tab 焦点帧（对齐 JSON 帧 201，非产品主窗）。 */
class QuickNoteColumnEditLatchCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun captureQuickNoteColumnEditLatchTabFocus() = runDesktopComposeUiTest(width = 360, height = 72) {
        ...
        waitForIdle()
        val file = File(dir, "203-compose-quicknote-column-edit-latch-tab-focus.png")
        val image = onRoot().captureToImage().toAwtImage()
        assertTrue(ImageIO.write(image, "png", file))
        assertTrue(countRingPixels(image, expected) >= 8, "quicknote column edit latch focus ring")
    }
