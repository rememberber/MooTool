...
/** F03 格式化「格式化」钮 Compose Tab 焦点帧（非产品主窗）。 */
class ReformatFormatTabCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun captureReformatFormatTabFocus() = runDesktopComposeUiTest(width = 360, height = 72) {
        ...
        waitForIdle()
        val file = File(dir, "190-compose-reformat-format-tab-focus.png")
        val image = onRoot().captureToImage().toAwtImage()
        assertTrue(ImageIO.write(image, "png", file))
        assertTrue(countRingPixels(image, expected) >= 8, "reformat format focus ring")
    }
