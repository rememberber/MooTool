...
/** F02 文本对比高亮选项行（对齐 [TextDiffScreen] `.diff-toolbar__options`）Compose Tab 焦点帧。 */
class TextDiffHighlightModeCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun captureDiffToolbarOptionsTabFocus() = runDesktopComposeUiTest(width = 920, height = 120) {
        ...
        setContent {
            MooTheme(preference = ThemePreference.Light, systemDark = false, interfaceStyle = "modern") {
                ...
                Row(
                    ...
                    MooButton(zh.t("diff.ignoreWhitespace"), p5Toolbar = true, onClick = {})
                    MooButton(
                        zh.t("diff.highlightBoth"),
                        prominent = true,
                        p5Toolbar = true,
                        onClick = {},
                        modifier = Modifier.focusRequester(focus),
                    )
                    MooButton(zh.t("diff.highlightCharacters"), p5Toolbar = true, onClick = {})
                }
            }
        }
        ...
        waitForIdle()
        val file = File(dir, "168-compose-text-diff-highlight-options-tab-focus.png")
        val image = onRoot().captureToImage().toAwtImage()
        assertTrue(ImageIO.write(image, "png", file))
        assertTrue(countRingPixels(image, expected) >= 8, "text diff highlight options focus ring")
    }
