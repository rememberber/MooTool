...
/** F04 JSON 检查器 JSONPath「复制」按钮 Compose Tab 焦点帧（非产品主窗）。 */
class JsonInspectorCopyPathCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun captureJsonInspectorCopyPathButtonTabFocus() = runDesktopComposeUiTest(width = 320, height = 120) {
        ...
        waitForIdle()
        val file = File(dir, "166-compose-json-inspector-copy-path-tab-focus.png")
        val image = onRoot().captureToImage().toAwtImage()
        assertTrue(ImageIO.write(image, "png", file))
        assertTrue(countRingPixels(image, expected) >= 8, "json inspector copy path focus ring")
    }
