...
/** F09 HTTP「发送」钮 Compose Tab 焦点帧（非产品主窗）。 */
class HttpSendButtonCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun captureHttpSendTabFocus() = runDesktopComposeUiTest(width = 400, height = 88) {
        ...
        waitForIdle()
        val file = File(dir, "184-compose-http-send-tab-focus.png")
        val image = onRoot().captureToImage().toAwtImage()
        assertTrue(ImageIO.write(image, "png", file))
        assertTrue(countRingPixels(image, expected) >= 8, "http send focus ring")
    }
