...
/** F21 计算器「=」钮 Compose Tab 焦点帧（非产品主窗）。 */
class CalculatorEvaluateTabCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun captureCalculatorEvaluateTabFocus() = runDesktopComposeUiTest(width = 360, height = 88) {
        ...
        waitForIdle()
        val file = File(dir, "189-compose-calculator-evaluate-tab-focus.png")
        val image = onRoot().captureToImage().toAwtImage()
        assertTrue(ImageIO.write(image, "png", file))
        assertTrue(countRingPixels(image, expected) >= 8, "calculator evaluate focus ring")
    }
