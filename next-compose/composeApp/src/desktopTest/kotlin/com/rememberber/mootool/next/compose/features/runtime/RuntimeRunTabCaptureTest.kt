...
/** F05 代码运行「运行」钮 Compose Tab 焦点帧（非产品主窗）。 */
class RuntimeRunTabCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun captureRuntimeRunTabFocus() = runDesktopComposeUiTest(width = 360, height = 72) {
        ...
        waitForIdle()
        val file = File(dir, "187-compose-runtime-run-tab-focus.png")
        val image = onRoot().captureToImage().toAwtImage()
        assertTrue(ImageIO.write(image, "png", file))
        assertTrue(countRingPixels(image, expected) >= 8, "runtime run focus ring")
    }
