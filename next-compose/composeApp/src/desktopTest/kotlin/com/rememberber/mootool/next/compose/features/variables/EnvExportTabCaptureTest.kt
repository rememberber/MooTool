...
/** F08 环境变量「导出」钮 Compose Tab 焦点帧（非产品主窗）。 */
class EnvExportTabCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun captureEnvExportTabFocus() = runDesktopComposeUiTest(width = 360, height = 72) {
        ...
        waitForIdle()
        val file = File(dir, "194-compose-env-export-tab-focus.png")
        val image = onRoot().captureToImage().toAwtImage()
        assertTrue(ImageIO.write(image, "png", file))
        assertTrue(countRingPixels(image, expected) >= 8, "env export focus ring")
    }
