...
/** F25 硬件刷新钮 Compose Tab 焦点帧（非产品主窗）。 */
class HardwareRefreshTabCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun captureHardwareRefreshTabFocus() = runDesktopComposeUiTest(width = 360, height = 72) {
        ...
        waitForIdle()
        val file = File(dir, "186-compose-hardware-refresh-tab-focus.png")
        val image = onRoot().captureToImage().toAwtImage()
        assertTrue(ImageIO.write(image, "png", file))
        assertTrue(countRingPixels(image, expected) >= 8, "hardware refresh focus ring")
    }
