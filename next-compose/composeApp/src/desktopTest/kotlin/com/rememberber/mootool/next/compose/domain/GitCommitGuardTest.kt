...
/** 未解决冲突时 commit 引擎拒绝（对齐 Electron `VaultGitService.commit` 与 Vault Git 面板 commit 禁用）。 */
class GitCommitGuardTest {
    @Test
    fun commitBlockedWhileMergeConflictsRemain() {
        assumeGit()
        val root = Files.createTempDirectory("mootool-compose-git-commit-guard-")
        try {
            ...
            assertTrue(
                blocked.message.contains("checkpoint", ignoreCase = true) ||
                    blocked.message.contains("merge", ignoreCase = true) ||
                    blocked.message.contains("conflict", ignoreCase = true),
                blocked.message,
            )
        } finally {
