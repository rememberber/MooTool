...
/** F14 加解密「验签」钮 Compose Tab 焦点帧（非产品主窗）。 */
class CryptoVerifyTabCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun captureCryptoVerifyTabFocus() = runDesktopComposeUiTest(width = 360, height = 88) {
        ...
        waitForIdle()
        val file = File(dir, "197-compose-crypto-verify-tab-focus.png")
        val image = onRoot().captureToImage().toAwtImage()
        assertTrue(ImageIO.write(image, "png", file))
        assertTrue(countRingPixels(image, expected) >= 8, "crypto verify focus ring")
    }
