...
/** F09 HTTP 响应区标题行（对齐 [HttpScreen] 响应 Tab 行）Compose Tab 焦点帧。 */
class HttpResponseHeadCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun captureHttpResponseFindButtonTabFocus() = runDesktopComposeUiTest(width = 820, height = 120) {
        ...
        setContent {
            MooTheme(preference = ThemePreference.Light, systemDark = false, interfaceStyle = "modern") {
                ...
                Row(
                    ...
                    MooButton(zh.t("common.action.copy"), p5Toolbar = true, onClick = {})
                }
            }
        }
        ...
        waitForIdle()
        val file = File(dir, "162-compose-http-response-head-tab-focus.png")
        val image = onRoot().captureToImage().toAwtImage()
        assertTrue(ImageIO.write(image, "png", file))
        assertTrue(countRingPixels(image, expected) >= 8, "http response head focus ring")
    }
