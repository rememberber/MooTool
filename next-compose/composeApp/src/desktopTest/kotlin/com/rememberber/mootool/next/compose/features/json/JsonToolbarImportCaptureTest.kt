...
/** F04 JSON 工具栏 `.io-actions` 导入钮 Compose Tab 焦点帧（非产品主窗）。 */
class JsonToolbarImportCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun captureJsonToolbarImportTabFocus() = runDesktopComposeUiTest(width = 280, height = 100) {
        ...
        setContent {
            MooTheme(preference = ThemePreference.Light, systemDark = false, interfaceStyle = "modern") {
                ...
                Row(
                    ...
                    MooButton(zh.t("json.action.export"), p5Toolbar = true, onClick = {})
                }
            }
        }
        ...
        waitForIdle()
        val file = File(dir, "173-compose-json-toolbar-import-tab-focus.png")
        val image = onRoot().captureToImage().toAwtImage()
        assertTrue(ImageIO.write(image, "png", file))
        assertTrue(countRingPixels(image, expected) >= 8, "json toolbar import focus ring")
    }
