...
/** Vault Git merge「继续合并」钮（§B 收尾）Compose Tab 焦点帧（非产品主窗）。 */
class GitMergeContinueCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun captureMergeContinueButtonTabFocus() = runDesktopComposeUiTest(width = 480, height = 120) {
        ...
        waitForIdle()
        val file = File(dir, "199-compose-git-merge-continue-tab-focus.png")
        val image = onRoot().captureToImage().toAwtImage()
        assertTrue(ImageIO.write(image, "png", file))
        assertTrue(countRingPixels(image, expected) >= 8, "git merge continue focus ring")
    }
