...
/** F24 PDF 工具栏操作行 Compose Tab 焦点帧（非产品主窗）。 */
class PdfToolbarActionsCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun capturePdfToolbarMergeTabFocus() = runDesktopComposeUiTest(width = 320, height = 100) {
        ...
        setContent {
            MooTheme(preference = ThemePreference.Light, systemDark = false, interfaceStyle = "modern") {
                ...
                Row(
                    ...
                    MooButton(zh.t("pdf.startSplit"), p5Toolbar = true, onClick = {})
                }
            }
        }
        ...
        waitForIdle()
        val file = File(dir, "177-compose-pdf-toolbar-merge-tab-focus.png")
        val image = onRoot().captureToImage().toAwtImage()
        assertTrue(ImageIO.write(image, "png", file))
        assertTrue(countRingPixels(image, expected) >= 8, "pdf toolbar merge focus ring")
    }
