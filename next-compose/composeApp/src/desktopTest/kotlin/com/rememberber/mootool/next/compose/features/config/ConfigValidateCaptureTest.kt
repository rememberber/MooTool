...
/** F06 YAML 校验 Tab `.validate-actions` 按钮 Compose Tab 焦点帧（非产品主窗）。 */
class ConfigValidateCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun captureConfigValidateButtonTabFocus() = runDesktopComposeUiTest(width = 200, height = 140) {
            ...
            MooTheme(preference = ThemePreference.Light, systemDark = false, interfaceStyle = "modern") {
                ...
                Column(
                    Modifier
                        ...
                        .padding(12.dp)
                        .mooConfigValidateActions(),
                    verticalArrangement = Arrangement.spacedBy(8.dp),
                ) {
                    MooButton(
                        zh.t("config.validate"),
                        prominent = true,
                        p5Toolbar = true,
        ...
        waitForIdle()
        val file = File(dir, "165-compose-config-validate-tab-focus.png")
        val image = onRoot().captureToImage().toAwtImage()
        assertTrue(ImageIO.write(image, "png", file))
        assertTrue(countRingPixels(image, expected) >= 8, "config validate button focus ring")
    }
