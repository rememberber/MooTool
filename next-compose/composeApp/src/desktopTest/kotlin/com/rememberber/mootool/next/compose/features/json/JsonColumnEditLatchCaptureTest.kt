...
/** F04 JSON「列编辑」闩锁钮 Compose Tab 焦点帧（对齐 IME 走查入口，非产品主窗）。 */
class JsonColumnEditLatchCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun captureJsonColumnEditLatchTabFocus() = runDesktopComposeUiTest(width = 360, height = 72) {
        ...
        waitForIdle()
        val file = File(dir, "201-compose-json-column-edit-latch-tab-focus.png")
        val image = onRoot().captureToImage().toAwtImage()
        assertTrue(ImageIO.write(image, "png", file))
        assertTrue(countRingPixels(image, expected) >= 8, "json column edit latch focus ring")
    }
