...
/** Vault Git rebase ours/theirs 行（§C 与 merge 共用 [VaultGitDialog] 解析钮）Compose 焦点帧。 */
class GitRebaseResolveCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun captureRebaseResolveOursButtonFocusRing() = runDesktopComposeUiTest(width = 640, height = 120) {
            ...
            MooTheme(preference = ThemePreference.Light, systemDark = false, interfaceStyle = "modern") {
                ...
                Column(
                    ...
                    Row(
                        ...
                        modifier = Modifier.mooGitMergeResolveRow(),
                    ) {
                        MooButton(
                            zh.t("git.ours"),
                            p5Toolbar = true,
        ...
        waitForIdle()
        val file = File(dir, "210-compose-git-rebase-resolve-tab-focus.png")
        val image = onRoot().captureToImage().toAwtImage()
        assertTrue(ImageIO.write(image, "png", file))
        assertTrue(countRingPixels(image, expected) >= 8, "git rebase resolve ours focus ring")
    }
