...
/** 未解决冲突时 pull 禁用（DIFF-520；fetch 仍可用，见 DIFF-619）。 */
class GitVaultPullDisabledConflictsCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun capturePullDisabledWhenConflictsRemain() = runDesktopComposeUiTest(width = 480, height = 120) {
        ...
        dir.mkdirs()
        waitForIdle()
        val file = File(dir, "217-compose-git-pull-disabled-unresolved-conflicts-tab-focus.png")
        val image = onRoot().captureToImage().toAwtImage()
        assertTrue(ImageIO.write(image, "png", file))
        assertTrue(file.length() > 500, "pull disabled with conflicts frame should be non-empty")
    }
}
