...
/** F04 JSON 检查器「生成 JSON Schema」钮 Compose Tab 焦点帧（非产品主窗）。 */
class JsonInspectorInferSchemaCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun captureJsonInspectorInferSchemaTabFocus() = runDesktopComposeUiTest(width = 360, height = 120) {
        ...
        waitForIdle()
        val file = File(dir, "198-compose-json-inspector-infer-schema-tab-focus.png")
        val image = onRoot().captureToImage().toAwtImage()
        assertTrue(ImageIO.write(image, "png", file))
        assertTrue(countRingPixels(image, expected) >= 8, "json inspector infer schema focus ring")
    }
