...
/** F14 加解密「生成密钥对」钮 Compose Tab 焦点帧（非产品主窗）。 */
class CryptoGenerateKeyTabCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun captureCryptoGenerateKeyTabFocus() = runDesktopComposeUiTest(width = 420, height = 88) {
        ...
        waitForIdle()
        val file = File(dir, "188-compose-crypto-generate-key-tab-focus.png")
        val image = onRoot().captureToImage().toAwtImage()
        assertTrue(ImageIO.write(image, "png", file))
        assertTrue(countRingPixels(image, expected) >= 8, "crypto generate key focus ring")
    }
