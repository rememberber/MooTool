...
/** Vault Git merge ours/theirs 行（对齐 [VaultGitDialog] 冲突解析钮）Compose 焦点帧。 */
class GitMergeResolveCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun captureMergeResolveOursButtonFocusRing() = runDesktopComposeUiTest(width = 640, height = 120) {
        ...
        waitForIdle()
        val file = File(dir, "158-compose-git-merge-resolve-tab-focus.png")
        val image = onRoot().captureToImage().toAwtImage()
        assertTrue(ImageIO.write(image, "png", file))
        assertTrue(countRingPixels(image, expected) >= 8, "git merge resolve ours focus ring")
    }
