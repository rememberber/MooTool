...
/** F18 时间转换「转本地时间」钮 Compose Tab 焦点帧（非产品主窗）。 */
class TimeConvertActionsCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun captureTimeToLocalTabFocus() = runDesktopComposeUiTest(width = 360, height = 72) {
        ...
        waitForIdle()
        val file = File(dir, "180-compose-time-to-local-tab-focus.png")
        val image = onRoot().captureToImage().toAwtImage()
        assertTrue(ImageIO.write(image, "png", file))
        assertTrue(countRingPixels(image, expected) >= 8, "time toLocal focus ring")
    }
