...
/** F17 二维码预览区保存钮 Compose Tab 焦点帧（非产品主窗）。 */
class QrPreviewActionsCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun captureQrPreviewSaveTabFocus() = runDesktopComposeUiTest(width = 260, height = 100) {
        ...
        waitForIdle()
        val file = File(dir, "176-compose-qr-preview-save-tab-focus.png")
        val image = onRoot().captureToImage().toAwtImage()
        assertTrue(ImageIO.write(image, "png", file))
        assertTrue(countRingPixels(image, expected) >= 8, "qr preview save focus ring")
    }
