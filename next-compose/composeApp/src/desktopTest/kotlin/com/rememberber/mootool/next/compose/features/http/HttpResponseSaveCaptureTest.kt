...
/** F09 HTTP「另存响应」钮 Compose Tab 焦点帧（非产品主窗）。 */
class HttpResponseSaveCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun captureHttpResponseSaveTabFocus() = runDesktopComposeUiTest(width = 360, height = 72) {
        ...
        waitForIdle()
        val file = File(dir, "192-compose-http-response-save-tab-focus.png")
        val image = onRoot().captureToImage().toAwtImage()
        assertTrue(ImageIO.write(image, "png", file))
        assertTrue(countRingPixels(image, expected) >= 8, "http response save focus ring")
    }
