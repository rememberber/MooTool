...
/** F20 翻译工具栏「立即翻译」钮 Compose Tab 焦点帧（非产品主窗）。 */
class TranslationNowButtonCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun captureTranslationNowTabFocus() = runDesktopComposeUiTest(width = 420, height = 88) {
        ...
        waitForIdle()
        val file = File(dir, "179-compose-translation-now-tab-focus.png")
        val image = onRoot().captureToImage().toAwtImage()
        assertTrue(ImageIO.write(image, "png", file))
        assertTrue(countRingPixels(image, expected) >= 8, "translation now focus ring")
    }
