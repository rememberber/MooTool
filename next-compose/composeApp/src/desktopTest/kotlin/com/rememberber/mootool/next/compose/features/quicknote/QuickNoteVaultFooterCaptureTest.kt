...
/** F01 Vault 底栏重命名钮 Compose Tab 焦点帧（非产品主窗）。 */
class QuickNoteVaultFooterCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun captureQuickNoteVaultFooterRenameTabFocus() = runDesktopComposeUiTest(width = 320, height = 72) {
            ...
            MooTheme(preference = ThemePreference.Light, systemDark = false, interfaceStyle = "modern") {
                ...
                Row(
                    Modifier
                        ...
                        .mooQuickNoteVaultFooter()
                        .mooQuickNoteVaultFooterActions()
                        .padding(4.dp),
                    horizontalArrangement = Arrangement.spacedBy(4.dp),
                ) {
                    MooButton(
                        zh.t("quickNote.rename"),
                        onClick = {},
        ...
        waitForIdle()
        val file = File(dir, "181-compose-quicknote-vault-footer-rename-tab-focus.png")
        val image = onRoot().captureToImage().toAwtImage()
        assertTrue(ImageIO.write(image, "png", file))
        assertTrue(countRingPixels(image, expected) >= 8, "quicknote vault footer rename focus ring")
    }
