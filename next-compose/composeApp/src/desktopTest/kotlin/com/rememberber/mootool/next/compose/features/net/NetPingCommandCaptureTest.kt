...
/** F11 网络 PING 命令行 Compose Tab 焦点帧（非产品主窗）。 */
class NetPingCommandCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun captureNetPingCommandTabFocus() = runDesktopComposeUiTest(width = 420, height = 88) {
        ...
        waitForIdle()
        val file = File(dir, "185-compose-net-ping-command-tab-focus.png")
        val image = onRoot().captureToImage().toAwtImage()
        assertTrue(ImageIO.write(image, "png", file))
        assertTrue(countRingPixels(image, expected) >= 8, "net ping command focus ring")
    }
