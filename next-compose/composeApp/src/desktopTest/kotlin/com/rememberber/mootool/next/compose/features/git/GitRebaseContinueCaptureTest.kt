...
/** Vault Git rebase「继续合并 / Rebase」钮（§C 收尾）Compose Tab 焦点帧（非产品主窗）。 */
class GitRebaseContinueCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun captureRebaseContinueButtonTabFocus() = runDesktopComposeUiTest(width = 520, height = 120) {
        ...
        waitForIdle()
        val file = File(dir, "207-compose-git-rebase-continue-tab-focus.png")
        val image = onRoot().captureToImage().toAwtImage()
        assertTrue(ImageIO.write(image, "png", file))
        assertTrue(countRingPixels(image, expected) >= 8, "git rebase continue focus ring")
    }
