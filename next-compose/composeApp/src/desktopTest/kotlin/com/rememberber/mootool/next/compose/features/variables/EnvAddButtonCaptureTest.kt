...
/** F08 环境变量「添加」钮 Compose Tab 焦点帧（非产品主窗）。 */
class EnvAddButtonCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun captureEnvAddVariableTabFocus() = runDesktopComposeUiTest(width = 360, height = 88) {
        ...
        waitForIdle()
        val file = File(dir, "182-compose-env-add-variable-tab-focus.png")
        val image = onRoot().captureToImage().toAwtImage()
        assertTrue(ImageIO.write(image, "png", file))
        assertTrue(countRingPixels(image, expected) >= 8, "env add variable focus ring")
    }
