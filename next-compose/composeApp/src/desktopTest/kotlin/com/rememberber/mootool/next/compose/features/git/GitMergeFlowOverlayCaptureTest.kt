...
/** Vault Git merge 进行中 hint 叠层（对齐 [VaultGitDialog] 状态胶囊 + 冲突说明行）Compose 焦点帧。 */
class GitMergeFlowOverlayCaptureTest {
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun captureMergeFlowHintRefreshButtonFocusRing() = runDesktopComposeUiTest(width = 640, height = 160) {
            ...
            MooTheme(preference = ThemePreference.Light, systemDark = false, interfaceStyle = "modern") {
                ...
                Column(
                    ...
                    Row(
                        ...
                        MooStatusPill(zh.t("git.operationMerge"), kind = MooStatusKind.Error)
                        Text(
                    ...
                    GitMergeConflictPresentation.unresolvedHintKey(merging, conflicts, "merge")?.let { key ->
                        Text(zh.t(key), color = colors.danger, fontSize = 11.sp)
                    }
                    GitMergeProductFlowPresentation.productFlowHintKey(merging, conflicts, selectedConflict)?.let { key ->
                        Text(
                            zh.t(key),
        ...
        waitForIdle()
        val file = File(dir, "153-compose-git-merge-flow-hint-tab-focus.png")
        val image = onRoot().captureToImage().toAwtImage()
        assertTrue(ImageIO.write(image, "png", file))
        assertTrue(countRingPixels(image, expected) >= 8, "git merge flow hint refresh focus ring")
    }
