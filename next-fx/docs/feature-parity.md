...
> 全部条目均为待开发要求，当前没有已实现的 FX 功能。本清单整理仓库既有对齐规格并复核关键 Electron 源码；每项开发前仍须读取实际页面、算法、服务和测试，枚举细粒度控件并保存实测证据。状态不能从其他产品继承。
...
- 所有身份、版本、关于信息均来自 JavaFX 产品，不显示 Electron 1.1.4 或 Java 1.8.6 冒充本版本。
...
- Nginx 识别引号/注释/转义/块；XML/HTML 保留文本节点语义；Java 使用真正语法处理器。
...
- 点路径、数组路径、嵌套、字符串/布尔/数字/null、转义、排序和默认值按 `configTools.ts` 及 Java 工具冻结。
...
- 改动前呈现差异和备份；系统作用域按需受限提权；不要求常驻管理员启动。Unix 写入范围由平台适配明确指定，不能任意改写用户全部 shell 配置。
- 验收：隔离用户/VM 中新增→新进程读取→修改→删除；拒绝权限保持原内容；刷新信息不产生写入。
...
- Header 按协议处理；Electron 的同名 Header 合并有局限，JavaFX 应声明重复字段策略；不静默合并 Set-Cookie 破坏语义。
...
- “保存方案”只写 JavaFX 数据；“应用到系统”才修改全局 hosts。先 diff、备份，提交时校验系统文件未被别人改动。
...
- 验收：Electron/Java 导出样本在 JavaFX 消费及反向消费；错误 key、坏编码、空内容、大文件取消。仅“自己加密自己解密”不合格。
- 工具兼容算法清楚标注模式，不将这条路径用于产品自身凭据存储。
...
- 灾难性回溯必须可终止：独立 worker、时间和结果数上限；仅 Task.cancel(true)、线程 interrupt 或 Future 超时不够。
...
- `?`、范围、步长、L/#、周名称/数字和 year 过滤以实际 Electron/Java 可用样本建兼容表；不宣称当前 Electron 是完整 Quartz。
...
- 验收：生成→保存→文件识别、生成→剪贴板→识别、中文 URL/Logo/透明图、权限或剪贴板失败。

### F18 时间转换

布局：秒/毫秒与日期时间双向、时区、当前/快捷时间及全屏时钟。

- 显式单位优先，不以字符串长度覆盖用户选择；负时间戳、严格日期校验、闰年与 DST。
...
- 文案、主题、对齐、比例持久化。若 JavaFX 改为字素计数，记录 Unicode 差异并更新界面长度提示。
...
- EXIF 方向、透明度、色彩、最大像素/内存、批量限制、取消及临时文件清理；持久图片库属于 JavaFX。
...
- 批量任务按 Electron 的20项限制逐入口核验并实现；奇/偶/自定义拆分；按所选文件和页码范围顺序合并。
...
- 验收：已知多页文档拆分→合并，检查页数/顺序/文本可提取；取消无半成品，打开输出路径真实存在。
...
- 验收：与本机基本信息核对、真实刷新、权限失败、页切换释放轮询和大表滚动。
...
| about | JavaFX 产品名/版本、更新状态/下载校验、版本说明、许可和帮助 |
...
- 历史不得默认存储私钥、密码、认证 Header；保留行为需有产品内明确选择和遮蔽策略，作为 JavaFX 隐私改进记录。
...
每项至少记录：caseId、toolId、sourceProduct/sourceCommit/sourceFile、输入、选项、期望结果/错误、比较方法、JavaFX 差异编号。保存到本产品 fixtures；CI 不启动其他版本来动态生成“期望值”。

比较方法：确定输出比文本/bytes；结构允许差异比 AST/协议内容；随机化加密/QR/PDF 用跨实现消费；随机数/系统数据比不变量；窗口/输入法比实际交互和截图。

源实现错误时保留 `sourceObserved` 与 `fxExpected` 及理由，不能覆盖原记录让测试迎合错误实现。原版未有的改进要标为设计决定，不回写成源事实。


## 5. JavaFX 实施补充和细项记录

每个 Fxx 在开发时拆成 Fxx.01、Fxx.02 等动作行，保存到 `docs/evidence/<日期>-<阶段>/parity.md`：原控件/位置、输入选项、FX控件/服务、默认值、期望、测试case、平台结果、差异编号。关键项缺失时Fxx整体不能标已验收。

- 原生控件映射：Vault/集合用TreeView/ListView；参数、任务和系统数据用TableView；布局用SplitPane；代码用EditorHost；Markdown预览可用受控WebView。不要用截图、Canvas字形或普通Label代替可操作文字控件。
- 动态算法在后台执行，用户文字始终属于DocumentSession；工具切换、窗口分离、主题变化和运行输出刷新不能重建文本/撤销历史。
- 正则和查找正则统一显示Java引擎差异；不能为了JS兼容把整个应用数据暴露给脚本执行器。
- 旧Java算法复制后必须移除跨产品静态配置/DAO/窗口依赖；纯fixture也复制进本产品，不在CI中相对读取其他产品。
- 设置source schema=12不等于FX schema=12；FX从自有schema开始，兼容导入是显式转换操作。
- FX的页面标题/安装名称/更新提示始终来自本产品身份；工具栏可延续MooTool品牌，但“关于”和系统窗口不能让用户无法区分同时运行的产品。
- 数据保存、迁移、任务取消、快捷键、权限错误和安装共存都是功能完成条件；不存在“按钮齐全即通过”的降级口径。
