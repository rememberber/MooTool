...
export const runtimeMessages = defineMessages({
  'zh-CN': {
    ...
    'notice.stopping': '正在终止运行进程…', 'operation.run': '运行代码', 'field.arguments': '参数',
    'field.workingDirectory': '工作目录', 'placeholder.workingDirectory': '留空使用隔离临时目录', 'action.detect': '检测',
    'action.stop': '停止', 'action.run': '运行', 'status.notInstalled': '未安装或不在 PATH', 'pane.source': '源码 · {runtime}',
    'action.restore': '恢复示例', 'aria.source': '运行源码', 'pane.output': '输出', 'output.running': '进程正在运行…',
    'output.empty': '运行输出将在这里实时显示', 'footer.capabilities': '白名单运行时 · 不经过 shell · 30 秒超时 · 支持终止进程组',
  ...
  'en-US': {
    ...
    'notice.stopping': 'Stopping the runtime process…', 'operation.run': 'Run code', 'field.arguments': 'Arguments',
    'field.workingDirectory': 'Working directory', 'placeholder.workingDirectory': 'Leave empty to use an isolated temporary directory', 'action.detect': 'Detect',
    'action.stop': 'Stop', 'action.run': 'Run', 'status.notInstalled': 'Not installed or not in PATH', 'pane.source': 'Source · {runtime}',
    'action.restore': 'Restore sample', 'aria.source': 'Runtime source', 'pane.output': 'Output', 'output.running': 'Process is running…',
    'output.empty': 'Runtime output will stream here', 'footer.capabilities': 'Allowlisted runtimes · No shell · 30-second timeout · Process-group termination',
  ...
  'ja-JP': {
    ...
    'notice.stopping': '実行プロセスを停止中…', 'operation.run': 'コードを実行', 'field.arguments': '引数',
    'field.workingDirectory': '作業ディレクトリ', 'placeholder.workingDirectory': '空の場合は隔離された一時ディレクトリを使用', 'action.detect': '検出',
    'action.stop': '停止', 'action.run': '実行', 'status.notInstalled': '未インストールまたは PATH にありません', 'pane.source': 'ソース · {runtime}',
    'action.restore': 'サンプルを復元', 'aria.source': '実行ソース', 'pane.output': '出力', 'output.running': 'プロセスを実行中…',
    'output.empty': '実行出力はここへリアルタイム表示されます', 'footer.capabilities': '許可済みランタイム · シェル不使用 · 30 秒タイムアウト · プロセスグループ停止',
