...
impl<T: TransparentPtrType> SList<T> {
    ...
    /// This operation is `O(1)`.
    #[inline]
    #[doc(alias = "g_slist_prepend")]
    pub fn push_front(&mut self, item: T) {
        unsafe {
            let ptr = self.ptr.map(|p| p.as_ptr()).unwrap_or(ptr::null_mut());
            self.ptr = Some(ptr::NonNull::new_unchecked(ffi::g_slist_prepend(
                ptr,
    ...
    /// this operation is `O(n)`.
    #[inline]
    #[doc(alias = "g_slist_append")]
    pub fn push_back(&mut self, item: T) {
        unsafe {
            let ptr = self.ptr.map(|p| p.as_ptr()).unwrap_or(ptr::null_mut());
            self.ptr = Some(ptr::NonNull::new_unchecked(ffi::g_slist_append(
                ptr,
                *(&mut *mem::ManuallyDrop::new(item) as *mut T as *mut *mut T::GlibType)
                    as ffi::gpointer,
            )));
    ...
    pub fn reverse(&mut self) {
        unsafe {
            ...
            self.ptr = Some(ptr::NonNull::new_unchecked(ffi::g_slist_reverse(ptr)));
        }
    }
    ...
    #[inline]
    pub fn as_ptr(&self) -> *const ffi::GSList {
    ...
    /// Returns the underlying pointer.
    #[inline]
    pub fn as_mut_ptr(&mut self) -> *mut ffi::GSList {
        self.ptr.map(|p| p.as_ptr()).unwrap_or(ptr::null_mut())
    }
...
impl<T: TransparentPtrType> std::iter::FromIterator<T> for SList<T> {
    #[inline]
    fn from_iter<I: IntoIterator<Item = T>>(iter: I) -> Self {
        unsafe {
            ...
            let mut tail = list;
            for item in iter {
                let new_tail = ffi::g_slist_alloc();
...
impl<'a, T: TransparentPtrType> std::iter::IntoIterator for &'a SList<T> {
    type Item = &'a T;
...
impl<'a, T: TransparentPtrType> std::iter::IntoIterator for &'a mut SList<T> {
    type Item = &'a mut T;
...
impl<T: TransparentPtrType> std::iter::IntoIterator for SList<T> {
    type Item = T;
...
impl<T: TransparentPtrType> FromGlibContainer<<T as GlibPtrDefault>::GlibType, *const ffi::GSList>
    for SList<T>
{
    #[inline]
    ...
    unsafe fn from_glib_full_num(_ptr: *const ffi::GSList, _num: usize) -> Self {
        unimplemented!();
    }
}

impl<T: TransparentPtrType> FromGlibPtrContainer<<T as GlibPtrDefault>::GlibType, *mut ffi::GSList>
    for SList<T>
{
    #[inline]
...
impl<T: TransparentPtrType>
    FromGlibPtrContainer<<T as GlibPtrDefault>::GlibType, *const ffi::GSList> for SList<T>
{
    #[inline]
    ...
    unsafe fn from_glib_full(_ptr: *const ffi::GSList) -> Self {
        unimplemented!();
    }
}

impl<'a, T: TransparentPtrType + 'a> ToGlibPtr<'a, *mut ffi::GSList> for SList<T> {
    type Storage = PhantomData<&'a Self>;

    #[inline]
    fn to_glib_none(&'a self) -> Stash<'a, *mut ffi::GSList, Self> {
    ...
    #[inline]
    fn to_glib_full(&self) -> *mut ffi::GSList {
        self.clone().into_raw()
    }
}

impl<'a, T: TransparentPtrType + 'a> ToGlibPtr<'a, *const ffi::GSList> for SList<T> {
    type Storage = PhantomData<&'a Self>;

    #[inline]
    fn to_glib_none(&'a self) -> Stash<'a, *const ffi::GSList, Self> {
        Stash(self.as_ptr(), PhantomData)
    }
}

impl<'a, T: TransparentPtrType + 'a> ToGlibPtrMut<'a, *mut ffi::GSList> for SList<T> {
    type Storage = PhantomData<&'a mut Self>;

    #[inline]
    fn to_glib_none_mut(&'a mut self) -> StashMut<'a, *mut ffi::GSList, Self> {
...
impl<'a, T: TransparentPtrType> Iter<'a, T> {
    #[inline]
    fn new(list: &'a SList<T>) -> Iter<'a, T> {
        debug_assert_eq!(
            mem::size_of::<T>(),
...
impl<T: TransparentPtrType> IntoIter<T> {
    #[inline]
    fn new(list: SList<T>) -> IntoIter<T> {
        debug_assert_eq!(
            mem::size_of::<T>(),
            mem::size_of::<<T as GlibPtrDefault>::GlibType>()
        );
...
mod test {
    ...
    fn from_glib_full() {
        ...
        let mut list = unsafe {
            ...
            SList::<crate::DateTime>::from_glib_full(list)
        };
        ...
        let list = unsafe { SList::<crate::DateTime>::from_glib_full(ptr::null_mut()) };
        assert!(list.is_empty());
    }
    ...
    fn from_glib_container() {
        ...
        let mut list = unsafe {
            ...
            SList::<crate::DateTime>::from_glib_container(list)
        };
        assert!(!list.is_empty());
    ...
    fn from_glib_none() {
        ...
        let mut list = unsafe {
            ...
            let res = SList::<crate::DateTime>::from_glib_none(list);
    ...
    fn safe_api() {
        ...
        let mut list = items[1..3].iter().cloned().collect::<SList<_>>();
