...
- [MooTool Java 1.8.0](https://github.com/rememberber/MooTool/releases/tag/v1.8.0)
...
ワークフロー：`.github/workflows/build-installers.yml`

> 可能な限り各プラットフォームはネイティブ runner でビルドしてください。

機能：

- `workflow_dispatch` に対応
- `v*` タグ push で自動実行
- `v*` タグ時、Hosted runner でビルド：
  - `macos-14`：`mac-apple-silicon`
  - `windows-latest`：`windows-x64`
  - `ubuntu-latest`：`linux-x64`
- `mac-intel` は手動のみ（`self-hosted`、`macOS`、`X64` のセルフホスト runner）
- `actions/cache` で `downloads/jdks/` と `jdks/` をキャッシュ
- 成果物はアップロード前にリネーム（例：`MooTool-1.7.0-mac-intel.dmg`、`MooTool-1.7.0-windows-x64.zip`）
- Actions Summary に元ファイル名 → Release ファイル名の対照表を表示
- `v*` タグで GitHub Release を作成/更新し、インストーラーを添付

手動 **Build installers** ワークフローの `target` オプション：

- `all`：Hosted プラットフォーム + オプションで `mac-intel`
- `mac-intel`：セルフホスト Intel Mac のみ
- `mac-apple-silicon` / `windows-x64` / `linux-x64`：単一 Hosted プラットフォーム

手動実行で `release_tag`（例：`v1.7.0`）を指定すると、成功した成果物を既存 Release に追加できます。本番リリース後に `mac-intel` を補完するのに便利です。

`mac-intel` には Intel Mac にセルフホスト runner を登録し、次のラベルが必要です：
