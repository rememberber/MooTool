...
| クライアント | ワンクリック連携 |
| --- | --- |
| Codex | MCP、Skill、または両方 |
| Claude Code | MCP、Skill、または両方 |
| Cursor | MCP |

- **11 個のツール**：JSON 整形・JSONPath 検索、エンコード・デコード、タイムスタンプ、テキスト比較、ハッシュ、UUID、およびクイックノートと JSON 文書の検索・読み取り。
- **連携の管理**：設定プレビュー、接続テスト、インストール状態の確認、修復・更新・削除に対応。既存の設定をバックアップし、ユーザーの編集との競合を検出して保持します。
- **文書の読み取り専用アクセス**：初期状態では無効で、文書庫ごとに許可します。許可の解除は次の呼び出しから適用され、文書庫の場所を変えると許可も解除されます。ページ分割に対応し、隠しファイル、シンボリックリンク、文書庫直下の `.gitignore` による除外ファイルは公開しません。

使い始めるには：

1. macOS の「アプリケーション」、Windows インストーラー、Linux deb を使い、固定の場所にインストールします。AppImage と Windows portable のランタイムはワンクリック AI 連携に登録できません。
2. **設定 → AI 連携** でクライアントと方式を選び、プレビューを確認してワンクリックでインストールします。
3. AI クライアントを再起動するか MCP / Skills を再読み込みし、必要に応じてツールを有効にします。例えば「MooTool でこの JSON のキーを並べ替えて」と依頼できます。
4. 保存済み文書を使う場合は、同じ画面で **クイックノートの読み取りを許可** または **JSON 文書の読み取りを許可** を有効にして、対応する文書庫の検索を依頼します。

他のローカル stdio MCP クライアントには MCP 設定のコピー機能を使えます。インストール先、制限、単独 Skill の使い方、検証記録は [AI 連携ガイド（中国語）](next/doc/mootool-ai-integration.md) をご覧ください。
...
Flutter、Compose、FX のソースビルドについては、上の製品表にある README をご覧ください。開発版のバージョン番号は、公開安定版のインストーラーがあることを意味しません。
