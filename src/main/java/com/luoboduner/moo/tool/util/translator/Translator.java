...
public interface Translator {
    ...
    static String[] getSourceLanguageNames() {
        return languageNameToCodeMap.keySet().toArray(new String[0]);
    }

    static String[] getTargetLanguageNames() {
        List<String> names = new ArrayList<>();
        for (String name : languageNameToCodeMap.keySet()) {
            if (!AUTO_DETECT.equals(name)) {
                names.add(name);
            }
        }
    ...
    static String resolveLanguageCode(String languageName, String defaultCode) {
        String code = languageNameToCodeMap.get(languageName);
